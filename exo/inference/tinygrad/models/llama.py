...
def sample_logits(logits: Tensor, temp: float, k: int, p: float, af: float, ap: float):
    ...
    if af or ap:
        ...
        logits = logits - (
            sample_logits.alpha_counter * af + (sample_logits.alpha_counter > 0) * ap
        )
    ...
    if af or ap:
        sample_logits.alpha_counter = (counter == output_token).where(
            sample_logits.alpha_counter + 1, sample_logits.alpha_counter
        )
...
def fix_bf16(weights: Dict[Any, Tensor]):
    ...
    if getenv("SUPPORT_BF16", 1):
        # TODO: without casting to float16, 70B llama OOM on tinybox.
        return {
            k: v.cast(dtypes.float32).cast(dtypes.float16)
            if v.dtype == dtypes.bfloat16
            else v
            for k, v in weights.items()
        }
